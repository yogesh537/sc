...
public:
    ...
    void drawc(int x1, int y1, int r) {
        ...
        d = 3 - 2 * r;
        do {
            putpixel(x0 + x, y0 + y, 15); // Top-right
            putpixel(x0 - x, y0 + y, 15); // Top-left
            putpixel(x0 + x, y0 - y, 15); // Bottom-right
            putpixel(x0 - x, y0 - y, 15); // Bottom-left
            putpixel(x0 + y, y0 + x, 15); // Top-right
            putpixel(x0 - y, y0 + x, 15); // Top-left
            putpixel(x0 + y, y0 - x, 15); // Bottom-right
            putpixel(x0 - y, y0 - x, 15); // Bottom-left
            if (d <= 0) {
                d = d + 4 * x + 6;
            } else {
                d = d + 4 * (x - y) + 10;
...
int main() {
    ...
    int x, y, r, x1 = 0, y1 = 0, x2 = 0, y2 = 0, xmax, ymax, xmid, ymid, n, i;
    dcircle c;
